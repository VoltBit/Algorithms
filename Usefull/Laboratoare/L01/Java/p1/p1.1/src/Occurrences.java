...
public class Occurrences {
	...
	private int countOccurrences(int[] v, int key, int lower, int upper) {
		...
		if (v[m] == key) return 1 + countOccurrences(v, key, lower, m - 1) + countOccurrences(v, key, m + 1, upper);
		if (v[m] > key) return countOccurrences(v, key, lower, m - 1);
		if (v[m] < key) return countOccurrences(v, key, m + 1, upper);
