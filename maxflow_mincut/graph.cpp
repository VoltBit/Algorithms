...
void Graph::create_graph_matrix(const char* file_name, int* a, int* b){
	printf("Reading from: %s\n", file_name);
	std::ifstream in(file_name);
	in >> this->N;
	for(int i = 0; i < this->N; i++){
		for(int j = 0; j < this->N; j++){
			in >> this->adjacency_matrix[i][j];
		}
	}
	in >> *a >> *b;
	in.close();
...
void Graph::display_structure(){
	...
	if(this->storage_type == LIST){
		...
		if(this->edges.size() > 0){
			...
			for(mit = this->edges.begin(); mit != this->edges.end(); mit++){
