package pa_lab5.prob1;
import java.util.ArrayList;
import java.util.Scanner;
import static pa_lab5.prob1.Nim.Inf;
...
//        if(depth == 0)return init.; ???????????????
        ...
        int max = -Inf;
        int score;
        Pair<Integer, Move> aux = new Pair(null, null);
        for(Move mov : moves){
            aux = minimax(init, player, (depth - 1));
            if(-aux.first > max) max = aux.first;
        }
